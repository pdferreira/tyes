...
class matches extends AnyFunSpec:
  ...
  describe("A Variable term") {
    ...
    for term <- Seq(
      ...
      Range("p", "i", 0, Seq(Variable("e_i")), 0, Index.Number(5), Some(Constant(7)))
    ) do
  ...
  describe("A Range term") {
    ...
    describe("with unlimited bounds") {
      ...
      describe("and no seed") {
        ...
        val range = Range(function, cursor, 0, Seq(template), 0, Index.Variable(boundsVar), None)
        ...
        it("matches a range with matching template modulo their cursors") {
          val r1 = Range("f", "i", 0, Seq(Function("P", Variable("e_i"), Variable("y"))), 0, Index.Variable("k"), None)
          val r2 = Range("f", "j", 0, Seq(Function("P", Variable("e_j"), Constant(1))), 0, Index.Number(5), None)
        ...
        it("does not match a range in a way that substitutes its ranged-over variables") {
          val otherRange = Range("f", "j", 0, Seq(Function("P", Variable("e_j"), Constant(1))), 0, Index.Number(5), None)
          assert(range.matches(otherRange) == None)
        }

        it("does not match a range with a seed") {
          val otherRange = Range(function, cursor, 0, Seq(template), range.minIndex, range.maxIndex, Some(Constant(2)))
          assert(range.matches(otherRange) == None)
        }
      }
      ...
      describe("and a seed") {
        ...
        val range = Range("f", "i", holeArgIdx, argTemplates, 1, Index.Variable("k"), Some(seed))
        ...
        it("matches a term that matches the seed at the innermost position") {
          ...
          val term = termFoldRight1(function, concreteSeed, holeArgIdx, args)
          val argsSubst = Map.from(
            for
        ...
        it("matches a range with matching template modulo their cursors") {
          ...
          val r1 = Range("f", "i", holeArgIdx, r1Templates, 0, Index.Variable("k"), Some(Variable("y")))
          val r2 = Range("f", "j", holeArgIdx, r2Templates, 0, Index.Number(5), Some(Constant(1)))
        ...
        it("does not match a range with a non-matching seed") {
          val otherRange = Range(function, cursor, holeArgIdx, argTemplates, range.minIndex, range.maxIndex, Some(Constant(2)))
          assert(range.matches(otherRange) == None)
        }
      }

      describe("and a minimum of occurrences") {
        val range = Range("f", "i", 0, Seq(Function("C", Variable("t_i"))), 0, Index.Variable("k", min = 2), None)
    ...
    describe("with limited bounds") {
      ...
      describe("and no seed") {
        val range = Range(function, cursor, 0, Seq(template), 0, Index.Number(2), None)
      ...
      describe("and a seed") {
        val seed = Function("K", Variable("a"))
        val range = Range(function, cursor, 0, Seq(template), 1, Index.Number(3), Some(seed))
        val concreteSeed = Function("K", Constant(true))
  ...
  describe("A Function term") {
    ...
    it("matches a bounded Range term with matching function") {
      ...
      val rangeTerm = Range("f", "i", 0, Seq(Variable("e_i")), 5, Index.Number(7), None)
    ...
    it("doesn't match a bounded Range term with incompatible bounds") {
      ...
      val rangeTerm = Range("f", "i", 0, Seq(Function("C", Variable("e_i"))), 5, Index.Number(9), None)
    ...
    it("doesn't match a bounded Range term with a different function") {
      val argVars = Seq("a", "b", "c")
      val funTerm = termFoldLeft1("f", argVars.map(Variable(_)))
      val rangeTerm = Range("g", "i", 0, Seq(Variable("e_i")), 5, Index.Number(7), None)
